...
//var format_start = "^XA~SD10^FWB^FO200,30^ARN, 25,20^FD";
//var format_line1 = "^FS^FO120,70^A0,30,25^FD";
//var format_line2 = "^FS^FO160,105^BY2^BAN,85,Y,N,N^A0N,25,20^FD";
//var format_line3 = "^FS^FWB^FO200,70^A0N,25,20^FD";
//var format_line4 = "^FS^FO200, 230^A0N,25,20^FD";
...
function sendData(print_params)
{
	//showLoading("Printing...");
	checkPrinterStatus( function (text){
		if (text == "Ready to Print")
		{
      selected_printer.send(print_params, printComplete, printerError);
			// selected_printer.send(format_start + $('#entered_name').val() + format_end, printComplete, printerError);
			//selected_printer.send(format_start + format_area + format_line1 + format_fecha + format_line2 + format_codbar + format_line3 + format_dir + format_line4 +  format_afijo + format_end, printComplete, printerError);
		}
		else
		{
			printerError(text);
